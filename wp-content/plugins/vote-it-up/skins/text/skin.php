...
$skinname = 'Text'; //No effect yet, but its good to state
...
<span class="textcontainer"><span id="votecount<?php the_ID(); ?>"><?php echo $votes; ?></span></span>
<?php if ($user_ID != '') {
 if (!($user_login == get_the_author_login() && !get_option('voteiu_allowownvote'))) { ?>
	<?php if(!UserVoted($postID,$user_ID)) { ?>
		<span class="bartext" id="voteid<?php the_ID(); ?>">
			<a href="javascript:vote('votecount<?php the_ID(); ?>','voteid<?php the_ID(); ?>','<?php echo htmlentities(get_option('voteiu_aftervotetext')); ?>',<?php the_ID(); ?>,<?php echo $user_ID; ?>,'<?php echo VoteItUp_ExtPath(); ?>');"><?php echo get_option('voteiu_votetext'); ?></a><?php if (get_option('voteiu_sinktext') != '') { ?><a href="javascript:sink('votecount<?php the_ID(); ?>','voteid<?php the_ID(); ?>','<?php echo htmlentities(get_option('voteiu_aftervotetext')); ?>',<?php the_ID(); ?>,<?php echo $user_ID; ?>,'<?php echo VoteItUp_ExtPath(); ?>');"><?php echo get_option('voteiu_sinktext'); ?></a><?php } ?>
...
if (get_option('voteiu_allowguests') == 'true') { ?>
	<?php if(!GuestVoted($postID,md5($_SERVER['REMOTE_ADDR']))) { ?><span class="bartext" id="voteid<?php the_ID(); ?>">
			<a href="javascript:vote('votecount<?php the_ID(); ?>','voteid<?php the_ID(); ?>','<?php echo htmlentities(get_option('voteiu_aftervotetext')); ?>',<?php the_ID(); ?>,0,'<?php echo VoteItUp_ExtPath(); ?>');"><?php echo get_option('voteiu_votetext'); ?></a><?php if (get_option('voteiu_sinktext') != '') { ?><a href="javascript:sink('votecount<?php the_ID(); ?>','voteid<?php the_ID(); ?>','<?php echo htmlentities(get_option('voteiu_aftervotetext')); ?>',<?php the_ID(); ?>,0,'<?php echo VoteItUp_ExtPath(); ?>');"><?php echo get_option('voteiu_sinktext'); ?></a><?php } ?>
